			...
			<div class="col-md-6" id="content-body">
				<div class="p-3 p-md-5">
				 ...
				 int id = Integer.parseInt(request.getParameter("songid"));
					...
					ArrayList<Song> songlist = dbutill.getSong(id);
					...
					<form class="" role="form"
						action="UpdateSong" method="post">
						<input type="hidden" name="sid" value=<%=song.getsID() %>>
						<div class="form-group">
							<label>Name</label><input type="text" name="name" value=<%=song.getsName() %>  class="form-control"
								placeholder="Enter Name">
						</div>
						<div class="form-group">
							<label>Album</label><input type="text" name="album" value=<%=song.getsAlbum() %> class="form-control"
								placeholder="Album">
						
						<div class="form-group">
								<label>ReleasedDate</label> <input type="text"  name="date" value=<%=song.getsReleasedDate() %> class="form-control"
									placeholder="Enter ReleasedDate">
							</div>
						
						<div class="form-group">
								<label>Price</label> <input type="text"  name="price" value=<%=song.getsPrice()%> class="form-control"
